...
st.caption("Objetivo de lectura: promedio semanal neto de enero, febrero, marzo y promedio de 3 meses.")
...
st.caption("Estos gráficos usan el mismo universo filtrado del dashboard para facilitar el seguimiento de productos, supermercados y sucursales.")

tendencia_productos = construir_tendencia_productos(df_filtrado)
mix_productos_supermercado = construir_mix_productos_supermercado(df_filtrado)
...
with v2:
    metrica_vis_options = ["Prom. 3 meses"] + [f"Prom. semanal neto {label}" for label in labels_activos]
    metrica_vis_sel = st.selectbox(
        "Métrica del análisis",
        metrica_vis_options,
        index=0,
    )

ranking_clientes_producto = construir_supermercados_producto(df_filtrado, producto_vis_sel, metrica_vis_sel)
evolucion_supermercados_producto = construir_evolucion_supermercados_producto(df_filtrado, producto_vis_sel)
ranking_sucursales_producto = construir_sucursales_producto(df_filtrado, producto_vis_sel, metrica_vis_sel)
...
with g1:
    ...
    if tendencia_productos.empty:
        ...
        fig_tendencia.update_layout(
            ...
            yaxis_title="Promedio semanal neto",
            xaxis_title="Mes",
