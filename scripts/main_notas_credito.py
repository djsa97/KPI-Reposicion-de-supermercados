...
# NAVEGACIÓN ERP
# =========================
def abrir_notas_credito(page) -> bool:
    ...
    debug("Abriendo REP. NOTAS CREDITOS...")
    if not click_text_safe(page, "REP. NOTAS CREDITOS", exact=True, timeout=15000):
        return False

    page.wait_for_timeout(4000)

    # Validaciones flexibles
    if page.get_by_role("button", name="REALIZAR CONSULTA").count() > 0:
        return True

    if page.locator("input").count() >= 2 and page.locator("text=FECHA DESDE").count() > 0:
        return True

    if page.locator("text=REPORTE DE NOTAS DE CREDITO").count() > 0:
        return True
...
def obtener_campos_fecha(page):
    """
    Busca de forma robusta los dos inputs de fecha.
    """
    # Intento 1: por name
    try:
    ...
    # Intento 2: inputs tipo text/date visibles dentro del formulario
    try:
    ...
    # Intento 3: primeros dos inputs visibles de toda la pantalla
    try:
...
def generar_reporte(page) -> bool:
    ...
    try:
        ...
        ok1 = fill_input_robusto(campo_desde, fecha_desde_ui, FECHA_DESDE)
        page.wait_for_timeout(300)
        ok2 = fill_input_robusto(campo_hasta, fecha_hasta_ui, FECHA_HASTA)
...
# =========================
# PARSER NOTAS DE CRÉDITO
# =========================
PATRON_CLIENTE = re.compile(r"^Cliente\s*:\s*(.+)$")
PATRON_FECHA = re.compile(r"Fecha\s*:\s*(\d{2}/\d{2}/\d{4})")
PATRON_PRODUCTO = re.compile(r"^(?P<producto>.+?)\s+(?P<cantidad>\d+)\s+(?P<pfinal>\d[\d\.]*)$")


def es_basura_nc(linea: str) -> bool:
    ...
    exactas = {
        ...
        "Criterios",
        "Producto Cantidad P.Final",
        "Producto",
        "Cantidad",
        "P.Final",
        "rep_notacredito_dev.jrxml",
    }
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            ...
            for linea in lineas:
                ...
                if linea.startswith("Obs."):
                    obs = linea.replace("Obs..", "").replace("Obs.", "").strip()
                    sucursal_actual = limpiar_obs_sucursal(obs)
                    continue

                if es_basura_nc(linea):
                    continue
                ...
                if m_prod and cliente_actual and fecha_actual:
                    ...
                    total = monto_a_int(m_prod.group("pfinal"))
...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if not abrir_notas_credito(page):
                debug("NAV FAIL")
                return

            debug("REP. NOTAS CREDITOS OK")
